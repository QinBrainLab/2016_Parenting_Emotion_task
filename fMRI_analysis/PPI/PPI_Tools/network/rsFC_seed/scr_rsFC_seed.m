...
%     pfolder = ['20', subjects{cnt}(1:2)];
     sessionlink_dir{cnt} = fullfile(raw_server,  ....
        subjects{cnt}, 'fmri', sess_folder);
    
    imagedir{cnt} = fullfile(raw_server, subjects{cnt}, ...
        'fmri', sess_folder, preprocessed_folder);
    
    mvmntdir{cnt,1} = fullfile(raw_server,subjects{cnt}, ...
        'fmri', sess_folder, 'unnormalized');
    
    mvmntdir{cnt,2} = fullfile(raw_server, subjects{cnt}, ...
        'fmri', sess_folder, preprocessed_folder);
    
    subject_dir{cnt} = fullfile(output_folder,  ...
